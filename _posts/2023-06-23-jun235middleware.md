...
// 이미지 업로드 같은 작업은 특정 라우터에만 일어나기 때문에
// app.use()에는 장착하지 않을 것
app.post('/upload', upload.single('image'), (req, res) => {
  // upload.single('image')의 업로드 정보가 req.file에 들어감
  // <input type="text" name="title" />의 텍스트 정보가 req.body에 들어감
  console.log(req.file, req.body);

  // 주소로 불러올 때는 http:// 프로토콜 꼭 넣어주기!!
  res.send(
    '<img src=http://localhost:3000/uploads/' + req.file.originalname + '>'
  );
});

app.listen(3000, () => {
  console.log('3000번 포트에서 동작중~');
});
...
![image-20230623174416980](/images/2023-06-23-jun235middleware/image-20230623174416980.png)
